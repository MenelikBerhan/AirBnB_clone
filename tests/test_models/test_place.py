#!/usr/bin/python3
"""
`TestPlace` test case to test the `Place` class
"""
...
class TestPlace(unittest.TestCase):
    """Tests for 'Place' class attributes and methods"""

    def setUp(self):
        """Create instances of Place class for testcases"""
        self.a = Place()
        self.b = Place()
        self.c = Place()
    ...
    def test_class_init(self):
        ...
        self.assertTrue(all(type(obj) == Place for obj in self.objects))
        self.assertTrue(all(isinstance(obj, (Place, BaseModel))
                            for obj in self.objects))

    def test_init_method(self):
        """Tests __init__method"""
        a_1 = Place(**self.a.to_dict())
        a_2 = Place('1', '2', '3', **self.a.to_dict())
    ...
    def test_class_attributes(self):
        ...
        self.assertTrue(hasattr(Place, attr) for attr in attributes)
        ...
        self.assertTrue(all(getattr(obj, attr) == 0 for obj in self.objects
                            for attr in attributes[4:8]))
        self.assertTrue(all(getattr(obj, attr) == 0.0 for obj in self.objects
                            for attr in attributes[8:-1]))
        self.assertTrue(all(getattr(obj, 'amenity_ids') == []
                            for obj in self.objects))
    ...
    def test_methods(self):
        ...
        self.assertTrue(hasattr(Place, method) for method in methods)
    ...
    def test_str(self):
        ...
        self.assertTrue(str(obj) == "[Place] ({obj.id}) {obj.__dict__}"
                        for obj in self.objects)
